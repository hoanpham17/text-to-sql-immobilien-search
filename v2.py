...
def generate_sql_prompt(schema_info_dict, user_query):
    ...
    template = ChatPromptTemplate.from_messages([
        SystemMessage(content=(
            ...
            '''3. Luôn sử dụng tên schema đầy đủ khi tham chiếu đến bảng (ví dụ: `raw.haus_mieten_test`), chú ý khi select luôn sử dụng dấu trích dẫn ví dụ  t."abCd","@id","@publishDate".\n'''
            ...
            "7. Với các cột dữ liệu json hãy đảm bảo bạn hiểu cấu trúc các phần tử con trong đó, nắm rõ đúng tên và giá trị của các cặp lable-value để có thể truy xuất chúng ra đúng\n"
            '''8. Đây là 1 ví dụ để truy vấn các đối tượng trong kiểu dữ liệu json, bạn có thể tham khảo để phát triển tìm các đối tượng khác nằm sâu hơn, hoặc phức tạp hơn
                ...
                WHERE (
                SELECT REPLACE(REPLACE(attr->>'value', '.', ''), ' €', '')::INTEGER
...
default_uri = "postgresql://postgres:123456@localhost:5432/immobilien"
...
/* Sử dụng data-testid chung cho stSlider và tìm class nội bộ của phần đã đầy */
...
/* Optional: thay đổi màu khi rê chuột hoặc đang kéo */
/* div[data-testid="stSlider"] div.st-dj:hover,
div[data-testid="stSlider"] div.st-dg:hover {
    background-color: #007bff !important;
    border-color: #007bff !important;
}
div[data-testid="stSlider"] div.st-dj:active,
div[data-testid="stSlider"] div.st-dg:active {
...
if st.session_state.schema_info:
    if st.session_state.schema_info:
        st.write("Cấu trúc schema database tìm thấy (không bao gồm schema hệ thống):")
            ...
            # Lặp qua từng schema
        for schema_name, tables in st.session_state.schema_info.items():
            # Chỉ hiển thị schema nếu nó có bảng (đã lọc bảng trống trong hàm load_schema)
            if tables: # Chỉ xử lý schema có bảng khả dụng
                # *** THAY ĐỔI Ở ĐÂY ***
                # Không dùng expander cho schema nữa, dùng tiêu đề
                st.subheader(f"Schema: **`{schema_name}`** ({len(tables)} bảng)") # <--- Dùng subheader cho Schema


                # Lặp qua từng bảng trong schema
                for table_name, columns in tables.items():
                    # *** THAY ĐỔI Ở ĐÂY ***
                    # Dùng expander cho từng bảng
                    # Đây là expander CẤP BẢNG, không lồng trong expander khác
                    with st.expander(f"Bảng: `{schema_name}.{table_name}` ({len(columns)} cột)"): # <--- Dùng Expander cho mỗi Bảng

                        if not columns:
                            st.write("Bảng này không có cột nào (hoặc không đọc được thông tin cột).")
                        else:
                            # *** THAY ĐỔI Ở ĐÂY ***
                            # Code hiển thị DataFrame Cột NẰM TRONG expander của Bảng này
                        ...
                        # Bỏ đường phân cách nếu mỗi bảng đã có expander riêng
